...
def lsb_to_volt(input_lsb):
    output_v = input_lsb * 2.5 / 2 ** 15
    return output_v


def attenuation(in_volt, freq):
    freq_att = 0.8799 - (3.7625 * 10 ** -9) * freq  # deal with changed due to freq

    out_volt = in_volt * 2.5 / freq_att
...
def ready_time(in_time):
    ...
    # n = 10**-9.
    time = in_time
    result = re.split(r"n|p", time, maxsplit=3)
